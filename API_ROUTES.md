...
**Ограничения:** 1 бесплатный запрос на пользователя. Для дополнительных запросов требуется подписка.
...
**Ошибки:**
- `400` - Профиль пользователя неполный (отсутствуют дата рождения, время или данные о местоположении)
- `403` - Лимит бесплатных запросов исчерпан (требуется подписка)
- `404` - Пользователь не найден
- `500` - Внутренняя ошибка сервера
...
## Обновленный маршрут: Да/Нет Таро

### Получение гадания Да/Нет

**POST** `/n8n/astro/yes-no-tarot`

Получает гадание Да/Нет на основе случайной карты Таро.
...
**Параметры:**
- `telegramId` (обязательный) - ID пользователя в Telegram
- `language` (опциональный) - язык ответа (по умолчанию "russian")
...
**Пример использования:**
```bash
curl -X POST http://localhost:3000/n8n/astro/yes-no-tarot \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_N8N_TOKEN" \
  -d '{
    "telegramId": "123456789",
...
**Примечание:** Этот маршрут использует внешний API https://json.astrologyapi.com/v1/yes_no_tarot для получения астрологических данных. Убедитесь, что в переменных окружения настроены `ASTROLOGY_API_USER_ID` и `ASTROLOGY_API_KEY`.
